...
def plot_by_augmentation_weight_with_constant_neighbors(n_neighbors=6):
    ...
    save_labeled_plot(trained_map, readable_labels,
                      make_results_path('constant_neighborhood_initialized', n_neighbors, augmentation_weight=0.0))
    ...
    for augmentation_weight in np.arange(0.0001, .002, 0.0001):
        ...
        trained_map = umap.UMAP(init=initial_embedding, metric='cosine', n_neighbors=n_neighbors,
                                random_state=42).fit(features_scaled)
        initial_embedding = trained_map.embedding_
        save_labeled_plot(trained_map, readable_labels,
                          make_results_path('constant_neighborhood_initialized', n_neighbors, augmentation_weight))
